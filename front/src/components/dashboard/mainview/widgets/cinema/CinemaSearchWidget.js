...
const CinemaSearchWidget = ({ widget, fetchUserWidgets }) => {
  ...
  const getSearch = useCallback(() => {
    movieService
      .getMovie(getParamValue(widget, 'Nom du film', 'Shutter Island'))
      .then(async response => {
      ...
      {search.data.results[0] ? (
        <div className={'flex'}>
          ...
          >
            ...
            >
              {search.data.results[0].title}
            </p>
            <p className={'mb-3 text-sm text-gray-600'}>
              {search.data.results[0].release_date}
            </p>
        ...
        <p>
          Ce film n&apos;existe pas ou il y a eu une erreur lors de la requête.
        </p>
      )}
    </Widget>
  ) : (
...
export default CinemaSearchWidget;
